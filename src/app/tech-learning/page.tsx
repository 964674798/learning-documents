...
export default async function TechLearningPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold mb-6 text-gray-900 dark:text-white">技术学习</h1>
      
      <p className="text-gray-600 dark:text-gray-300 mb-8">
        这里包含各种编程语言和技术框架的学习笔记和总结。
      </p>
          ...
          >
            ...
            <ul className="space-y-2">
              {subcategoryDocs.map(doc => (
                <li key={doc.slug}>
                  <Link 
                    href={`/tech-learning/${subcategory.toLowerCase()}/${doc.date}`} 
      ...
      {Object.keys(docsBySubcategory).length === 0 && (
        <div className="text-center py-10">
          <p className="text-gray-500 dark:text-gray-400">暂无技术学习文档</p>
        </div>
      )}
    </div>
  );
